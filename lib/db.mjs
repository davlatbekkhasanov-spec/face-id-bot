...
export function saveStaffState(row) {
  getDb()
    .prepare(
      `INSERT INTO staff_state (staff_key, staff_name, status, session_start_ms, last_scan_ms, day_key, day_worked_ms, had_leave, last_leave_ms)
       VALUES (@staff_key, @staff_name, @status, @session_start_ms, @last_scan_ms, @day_key, @day_worked_ms, @had_leave, @last_leave_ms)
       ON CONFLICT(staff_key) DO UPDATE SET
         ...
         had_leave=excluded.had_leave, last_leave_ms=excluded.last_leave_ms`
    )
    .run(row);
}
