...
import { initPointsSchema, deleteDayPoints, resetAllPoints, syncMonthlyPoints } from "./points-db.mjs";
import { initAbsenceSchema } from "./absence-db.mjs";
...
export function initDb(dataDir) {
  ...
  try {
    db.prepare("SELECT first_in_ms FROM daily_stats LIMIT 1").get();
  } catch {
    db.exec("ALTER TABLE daily_stats ADD COLUMN first_in_ms INTEGER");
    db.exec("ALTER TABLE daily_stats ADD COLUMN last_out_ms INTEGER");
  }
  initPointsSchema();
  initAbsenceSchema();
  console.log("DB:", file);
...
/** employees.json dagi ism o'zgarganda SQLite dagi eski staff_name larni yangilash */
export function syncStaffNamesFromEmployees(employees = { staff: {} }) {
  const d = getDb();
  let updated = 0;
  for (const [key, s] of Object.entries(employees.staff || {})) {
    const name = `${s.firstName || ""} ${s.lastName || ""}`.trim();
    if (!name) continue;
    updated += d.prepare("UPDATE staff_state SET staff_name=? WHERE staff_key=? AND staff_name!=?").run(name, key, name).changes;
    updated += d.prepare("UPDATE daily_stats SET staff_name=? WHERE staff_key=? AND staff_name!=?").run(name, key, name).changes;
    updated += d.prepare("UPDATE monthly_stats SET staff_name=? WHERE staff_key=? AND staff_name!=?").run(name, key, name).changes;
    updated += d.prepare("UPDATE daily_points SET staff_name=? WHERE staff_key=? AND staff_name!=?").run(name, key, name).changes;
    updated += d.prepare("UPDATE monthly_points SET staff_name=? WHERE staff_key=? AND staff_name!=?").run(name, key, name).changes;
  }
  return updated;
}
...
export function finalizeDayDebt(
  staffKey,
  staffName,
  dk,
  pk,
  ...
  getDb()
    .prepare(
      `INSERT INTO daily_stats (day_key, period_key, staff_key, staff_name, worked_ms, debt_ms, first_in_ms, last_out_ms)
       VALUES (?,?,?,?,?,?,?,?)
       ON CONFLICT(day_key, staff_key) DO UPDATE SET
         worked_ms=excluded.worked_ms,
         debt_ms=excluded.debt_ms,
         staff_name=excluded.staff_name,
         first_in_ms=CASE WHEN excluded.first_in_ms IS NOT NULL THEN excluded.first_in_ms ELSE daily_stats.first_in_ms END,
         last_out_ms=CASE WHEN excluded.last_out_ms IS NOT NULL THEN excluded.last_out_ms ELSE daily_stats.last_out_ms END`
    )
    .run(dk, pk, staffKey, staffName, dayWorkedMs, debtMs, firstInMs, lastOutMs);
  return debtMs;
}

export function getPeriodTimesheetRows(periodKey) {
  return getDb()
    .prepare(
      `SELECT ds.day_key, ds.staff_key, ds.staff_name, ds.worked_ms,
              ds.first_in_ms, ds.last_out_ms,
              COALESCE(dp.late_ms, 0) AS late_ms,
              COALESCE(dp.debt_ms, ds.debt_ms, 0) AS debt_ms,
              COALESCE(dp.overtime_ms, 0) AS overtime_ms
       FROM daily_stats ds
       LEFT JOIN daily_points dp ON dp.day_key = ds.day_key AND dp.staff_key = ds.staff_key
       WHERE ds.period_key = ?
       ORDER BY ds.staff_key, ds.day_key`
    )
    .all(periodKey);
}

export function getTimesheetRowsInRange(fromKey, toKey) {
  return getDb()
    .prepare(
      `SELECT ds.day_key, ds.staff_key, ds.staff_name, ds.worked_ms,
              ds.first_in_ms, ds.last_out_ms,
       ...
       FROM daily_stats ds
       LEFT JOIN daily_points dp ON dp.day_key = ds.day_key AND dp.staff_key = ds.staff_key
       WHERE ds.day_key >= ? AND ds.day_key <= ?
       ORDER BY ds.staff_key, ds.day_key`
    )
    .all(fromKey, toKey);
}

export function listTimesheetMonths(limit = 12) {
  const fromDb = getDb()
    .prepare(
      `SELECT DISTINCT period_key AS ym
       FROM daily_stats
       ORDER BY ym DESC
       LIMIT ?`
    )
...
/** Oy balansi (ms): jami plus − jami minus */
export function getMonthBalanceMs(pk, staffKey) {
  const row = getDb()
    .prepare(
      `SELECT
         COALESCE(SUM(COALESCE(dp.overtime_ms, 0)), 0) AS ot,
         COALESCE(SUM(COALESCE(dp.debt_ms, ds.debt_ms, 0)), 0) AS debt
       FROM daily_stats ds
       LEFT JOIN daily_points dp ON dp.day_key = ds.day_key AND dp.staff_key = ds.staff_key
       WHERE ds.period_key = ? AND ds.staff_key = ?`
    )
    .get(pk, staffKey);
  return (row?.ot || 0) - (row?.debt || 0);
}
...
/** Hodimning ma'lumot bor kunlari (test tozalash uchun) */
export function listStaffDataDays(staffKey, limit = 15) {
  const d = getDb();
  const keys = new Set();
  for (const r of d
    .prepare("SELECT day_key FROM daily_stats WHERE staff_key=? ORDER BY day_key DESC LIMIT ?")
    .all(staffKey, limit)) {
    keys.add(r.day_key);
  }
  for (const r of d
    .prepare("SELECT day_key FROM daily_points WHERE staff_key=? ORDER BY day_key DESC LIMIT ?")
    .all(staffKey, limit)) {
    keys.add(r.day_key);
  }
  const st = getStaffState(staffKey);
  if (st?.day_key) keys.add(st.day_key);
  ...
  return [...keys]
    ...
    .map((day_key) => {
      const ds = d
        .prepare("SELECT worked_ms, debt_ms FROM daily_stats WHERE day_key=? AND staff_key=?")
        .get(day_key, staffKey);
      const dp = d
        .prepare("SELECT day_total FROM daily_points WHERE day_key=? AND staff_key=?")
        .get(day_key, staffKey);
      const live = st?.day_key === day_key ? st : null;
      return {
        day_key,
        worked_ms: ds?.worked_ms ?? live?.day_worked_ms ?? 0,
        debt_ms: ds?.debt_ms ?? 0,
        day_total: dp?.day_total ?? 0,
        in_now: live?.status === "in",
      };
    });
}

function staffDisplayName(staffKey, employees, fallback = "") {
...
/** Faqat bitta hodimning bitta kuni — test sessiyasini tozalash */
export function resetStaffDayData(staffKey, dayKey, employees = { staff: {} }) {
  return getDb().transaction(() => {
    const pk = periodKey(new Date(`${dayKey}T12:00:00+05:00`).getTime());
    const row = getDb()
      .prepare("SELECT staff_name FROM daily_stats WHERE day_key=? AND staff_key=?")
      .get(dayKey, staffKey);
    const st = getStaffState(staffKey);
    const name = staffDisplayName(staffKey, employees, row?.staff_name || st?.staff_name);

    const dailyRows = getDb()
      .prepare("DELETE FROM daily_stats WHERE day_key=? AND staff_key=?")
      .run(dayKey, staffKey).changes;
    const pointsRows = getDb()
      .prepare("DELETE FROM daily_points WHERE day_key=? AND staff_key=?")
      .run(dayKey, staffKey).changes;

    let stateReset = 0;
    if (st?.day_key === dayKey) {
      getDb().prepare("DELETE FROM staff_state WHERE staff_key=?").run(staffKey);
      stateReset = 1;
    }

    syncMonthlyForStaff(pk, staffKey, name);
    syncMonthlyPoints(pk, staffKey, name);

    return { dayKey, staffKey, staffName: name, dailyRows, pointsRows, stateReset };
  })();
}
...
export function deleteDayData(dayKey, employees = { staff: {} }) {
  return getDb().transaction(() => {
    ...
    for (const r of syncSet.values()) {
      ...
      syncMonthlyPoints(r.period_key, r.staff_key, r.staff_name || r.staff_key);
    }
