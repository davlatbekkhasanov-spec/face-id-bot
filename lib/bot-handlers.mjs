...
import { formatAbsenceResolvedMessage, sendAbsenceAlertDemo } from "./absence-alerts.mjs";
import {
...
export async function handleCallbackQuery(q, ctx) {
  ...
  if (data === "adm:abspick") {
    ctx.pending?.delete(uid);
    return tgSend(chatId, "⚠️ <b>Kelmadi test</b>\nHodimni tanlang — xabar shu chatga keladi:", {
  ...
  if (data.startsWith("adm:abssend:")) {
    ...
    const r = await sendAbsenceAlertDemo(ctx, chatId, key);
    if (!r.ok) return tgSend(chatId, `⚠️ ${r.error}`);
    return tgSend(chatId, `✅ <b>${r.name}</b> uchun kelmadi xabari yuborildi.`, {
...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (text === "⚠️ Kelmadi test") {
      return tgSend(adminChat, "⚠️ <b>Kelmadi test</b>\nHodimni tanlang — xabar shu chatga keladi:", {
        reply_markup: absenceTestPickKeyboard(employees),
      });
    }
...
export function adminWelcome() {
  return (
    ...
    "⚠️ Kelmadi test — o'zingizga kelmadi xabarini yuborish\n" +
