...
export async function handleCallbackQuery(q, ctx) {
  const { botToken, dataDir, employees, adminIds, groupChatId, tgAnswer, tgSend } = ctx;
  const uid = q.from?.id;
  ...
  if (data.startsWith("adm:in:")) {
    ...
    await sendManualAndStore(dataDir, botToken, groupChatId, r.card, employees);
    return tgSend(chatId, `✅ Guruhga yuborildi: ${r.card.staffName}`, {
