...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (pending?.mode === "fixday_day") {
      ...
      return tgSend(
        chatId,
        `📝 <b>Kun kiritish</b>\n👤 ${name}\n📅 ${fmtDayHuman(dk)}\n\n4️⃣ Keldi vaqtini yuboring:\n<code>09:15</code>`,
        { reply_markup: adminReportsReplyKeyboard() }
      );
    }

    if (pending?.mode === "fixday_keldi") {
      const keldi = normHHMM(text);
      if (!keldi) {
        return tgSend(chatId, "⚠️ Format: <code>09:15</code> (HH:MM)");
      }
      ctx.pending.set(uid, {
        mode: "fixday_ketdi",
        staffKey: pending.staffKey,
      ...
      return tgSend(
        chatId,
        `📝 Keldi: <b>${keldi}</b>\n\n5️⃣ Ketdi vaqtini yuboring:\n<code>21:00</code>`
      );
    }

    if (pending?.mode === "fixday_ketdi") {
      const ketdi = normHHMM(text);
      if (!ketdi) {
        return tgSend(chatId, "⚠️ Format: <code>21:00</code> (HH:MM)");
      }
      const staffKey = pending.staffKey;
      const dk = pending.dayKey;
      const staffName = staffReplyLabel(employees.staff?.[staffKey]);
      const r = saveManualDayKeldiKetdi(
        staffKey,
        staffName,
        dk,
        pending.keldi,
