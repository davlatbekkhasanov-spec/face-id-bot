...
function startTabelPick(ctx, uid, adminChat, tgSend) {
  const { keyboard, monthMap } = buildMonthPickKeyboard();
  ctx.pending?.set(uid, { mode: "tabel_month", monthMap });
  return tgSend(adminChat, startTabelPickMessage(), { reply_markup: keyboard });
}

async function sendTabelForRange(ctx, adminChat, tgSend, employees, fromKey, toKey, uid) {
  try {
    const report = await buildTimesheetForRange(employees, fromKey, toKey);
    ctx.pending?.delete(uid);
    if (!report.png) {
      return tgSend(adminChat, report.emptyMessage || "Ma'lumot yo'q.", {
        reply_markup: adminReportsReplyKeyboard(),
      });
    }
    return ctx.tgSendPhoto(adminChat, report.png, report.caption, {
      reply_markup: adminReportsReplyKeyboard(),
    });
  } catch (e) {
    console.warn("timesheet:", e.message);
    return tgSend(adminChat, `⚠️ Tabel xatosi: ${e.message}`, {
      reply_markup: adminReportsReplyKeyboard(),
    });
  }
}
...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (text === "📋 Tabel") {
      return startTabelPick(ctx, uid, adminChat, tgSend);
    }
    ...
    if (pending?.mode === "tabel_month") {
      ...
      if (!ym) return;
      const { keyboard, dayMap } = buildDayPickKeyboard(ym, { fromDay: 1 });
      ctx.pending.set(uid, { mode: "tabel_from", ym, dayMap });
      return tgSend(adminChat, `📋 <b>TABEL</b>\n\n📅 ${monthPickLabel(ym)}\n\n2️⃣ <b>Boshlanish kunini</b> tanlang:`, {
    ...
    if (pending?.mode === "tabel_from") {
      ...
      if (!fromDay) return;
      const { keyboard, dayMap } = buildDayPickKeyboard(pending.ym, { fromDay });
      ctx.pending.set(uid, { mode: "tabel_to", ym: pending.ym, fromDay, dayMap });
      return tgSend(
        adminChat,
    ...
    if (pending?.mode === "tabel_to") {
      ...
      return sendTabelForRange(ctx, adminChat, tgSend, employees, fromKey, toKey, uid);
    }
