...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (text === "📊 Hisobotlar") {
      return showReportsMenu(ctx, uid, chatId, tgSend);
    }

    if (text === "📆 Ma'lumot kunlari") {
      return sendDaysListReport(ctx, chatId);
    }

    if (text === "🗑 Ma'lumot o'chirish") {
      return startDeleteDayPick(ctx, uid, chatId, tgSend);
    }
    ...
    if (REPORT_BUTTONS[text]) {
      ...
      if (fallback) {
        return tgSend(chatId, fallback, { reply_markup: adminReportsReplyKeyboard() });
      }
    }
