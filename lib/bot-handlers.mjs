...
import { REPORT_BUTTONS, buildAdminReport } from "./admin-reports.mjs";
...
async function sendManual(ctx, chatId, key, intent, clockHHMM = null) {
  const { dataDir, botToken, notifyChatId, employees, tgSend } = ctx;
...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (text === "📊 Hisobotlar") {
      ctx.pending?.delete(uid);
      return tgSend(chatId, reportsWelcome(), {
    ...
    if (REPORT_BUTTONS[text]) {
      const report = buildAdminReport(REPORT_BUTTONS[text], employees);
      if (report) {
        return tgSend(chatId, report, { reply_markup: adminReportsReplyKeyboard() });
      }
    }
  ...
  if (/^\/start/i.test(text) && isRegisteredEmployee(uid, employees)) {
...
export function adminWelcome() {
  return (
    ...
    "📊 Hisobotlar — kim ishda, qarz, reyting"
  );
}
