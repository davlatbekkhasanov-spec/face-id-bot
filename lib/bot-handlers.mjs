...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (pending?.mode === "tabel_month") {
      ...
      if (!ym) return;
      const pick = buildPresetPickKeyboard();
      ctx.pending.set(uid, { mode: "tabel_preset", ym, presetMap: pick.presetMap });
      return tgSend(chatId, presetPickMessage(ym), { reply_markup: pickReplyMarkup(pick) });
    }

    if (pending?.mode === "tabel_preset") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      const presetKey = pending.presetMap?.[text];
      if (!presetKey) return;
      if (presetKey === "custom") {
        const pick = buildDayPickKeyboard(pending.ym);
        ctx.pending.set(uid, { mode: "tabel_from", ym: pending.ym, dayMap: pick.dayMap });
        return tgSend(chatId, `📋 <b>TABEL</b>\n\n📅 ${monthPickLabel(pending.ym)}\n\n3️⃣ <b>Boshlanish kunini</b> tanlang (1—31):`, {
          reply_markup: pickReplyMarkup(pick),
        });
      }
      const range = resolveTabelPreset(pending.ym, presetKey);
      if (!range?.fromKey || !range?.toKey) {
        return tgSend(chatId, "⚠️ Davr noto'g'ri.", { reply_markup: adminReportsReplyKeyboard() });
      }
      await tgSend(chatId, "⏳ Tabel tayyorlanmoqda...");
      return sendTabelForRange(ctx, chatId, tgSend, employees, range.fromKey, range.toKey, uid);
    }
    ...
    if (pending?.mode === "tabel_from") {
      ...
      return tgSend(
        chatId,
        `📋 <b>TABEL</b>\n\n📅 ${monthPickLabel(pending.ym)}\n▶️ Boshlanish: <b>${fromDay}</b>\n\n4️⃣ <b>Tugash kunini</b> tanlang (1—31):`,
        { reply_markup: pickReplyMarkup(pick) }
      );
    }
    ...
    if (pending?.mode === "tabel_to") {
      ...
      if (!toDay) return;
      if (toDay < pending.fromDay) {
        return tgSend(chatId, `⚠️ Tugash kuni (${toDay}) boshlanishdan (${pending.fromDay}) oldin bo'lmasin.`);
      }
