...
import { fmtDuration, dayKeyFromParts, dayKey, fmtDurationNorm } from "./period.mjs";
import {
  adminMainKeyboard,
  employeePickKeyboard,
  absenceTestPickKeyboard,
  actionKeyboard,
  manualTimeKeyboard,
...
import {
  applyExcusedAbsence,
  applyUnexcusedAbsence,
} from "./absence-db.mjs";
import { formatAbsenceResolvedMessage, sendAbsenceAlertManual } from "./absence-alerts.mjs";
import {
...
import {
  ...
  importKeldiKetdiCaption,
  restoreAllAttendanceData,
} from "./attendance-log.mjs";
import { importDeviceBackfillJson } from "./device-backfill.mjs";
import { formatRestoreReport } from "./attendance-restore.mjs";
import {
  saveManualDayKeldiKetdi,
  formatManualDaySavedMessage,
  normHHMM,
  startFixDayTimeState,
  buildFixDayTimePrompt,
  buildFixDayCompleteMessage,
} from "./manual-day-fix.mjs";

function isAdmin(uid, adminIds) {
  return adminIds.has(Number(uid));
}

function startFixDayPick(ctx, uid, chatId, tgSend, employees) {
  ctx.pending?.set(uid, { mode: "fixday_staff" });
  return tgSend(
    chatId,
    "📝 <b>Kun kiritish</b>\n\nFace ID dan qolib ketgan kun uchun keldi/ketdi.\n\n1️⃣ Hodimni tanlang:",
    { reply_markup: adminStaffReplyKeyboard(employees) }
  );
}

function startFixDayMonth(ctx, uid, chatId, tgSend, employees, staffKey) {
  const pick = buildMonthPickKeyboard({ withBack: true });
  ctx.pending?.set(uid, { mode: "fixday_month", staffKey, monthMap: pick.monthMap });
  const name = staffReplyLabel(employees.staff?.[staffKey]);
  return tgSend(
    chatId,
    `📝 <b>Kun kiritish</b>\n👤 ${name}\n\n2️⃣ Oyni tanlang:`,
...
function startTabelPick(ctx, uid, chatId, tgSend) {
  try {
    ...
    ctx.pending?.set(uid, { mode: "tabel_from_month", monthMap: pick.monthMap });
...
export async function handleCallbackQuery(q, ctx) {
  ...
  if (data === "adm:abspick") {
    ctx.pending?.delete(uid);
    return tgSend(chatId, "⚠️ <b>Kelmadi</b>\nKelmagan hodimni tanlang:", {
      reply_markup: absenceTestPickKeyboard(employees),
    });
  }

  if (data.startsWith("adm:abssend:")) {
    const key = data.slice(12);
    const r = await sendAbsenceAlertManual(ctx, chatId, key);
    if (!r.ok) return tgSend(chatId, `⚠️ ${r.error}`);
    return tgSend(chatId, `✅ <b>${r.name}</b> — kelmadi xabari yuborildi.`, {
  ...
  if (data.startsWith("abs:e:")) {
    const key = data.slice(6);
    const staff = employees.staff?.[key];
    if (!staff) return tgSend(chatId, "Hodim topilmadi.");
    const dk = dayKey();
    const name = staffReplyLabel(staff);
    applyExcusedAbsence(dk, key, name);
    const text = formatAbsenceResolvedMessage(name, "excused", dk);
    if (ctx.tgEdit && q.message?.message_id) {
      return ctx.tgEdit(chatId, q.message.message_id, text);
    }
    return tgSend(chatId, text);
  }

  if (data.startsWith("abs:u:")) {
    const key = data.slice(6);
    const staff = employees.staff?.[key];
    if (!staff) return tgSend(chatId, "Hodim topilmadi.");
    const dk = dayKey();
    const name = staffReplyLabel(staff);
    applyUnexcusedAbsence(dk, key, name, employees);
    const text = formatAbsenceResolvedMessage(name, "unexcused", dk);
    if (ctx.tgEdit && q.message?.message_id) {
      return ctx.tgEdit(chatId, q.message.message_id, text);
    }
    return tgSend(chatId, text);
...
async function tryImportForwardedKeldiKetdi(msg, employees, adminChat, tgSend) {
  const whenSec = msg.forward_date || msg.date;
  if (!whenSec) return false;
  const caption = String(msg.caption || msg.text || "").trim();
  if (!caption || !/KETDI|ИШДАН КЕТДИ/i.test(caption)) return false;

  const dk = dayKey(whenSec * 1000);
  const r = importKeldiKetdiCaption(caption, dk, employees);
  if (!r) {
    return tgSend(adminChat, "⚠️ Xabarni o'qib bo'lmadi. KETDI xabari (Kelgan/Ketgan) bo'lishi kerak.");
  }
  return tgSend(
    adminChat,
    `✅ <b>Tiklandi</b>\n👤 ${r.staffName}\n📅 ${fmtDayHuman(dk)}\n` +
      `📥 ${r.keldi} → 📤 ${r.ketdi}\n⏱ ${fmtDurationNorm(r.workedMs)}`,
    { reply_markup: adminReportsReplyKeyboard() }
  );
}

async function handleTelegramExportUpload(msg, ctx, employees, adminChat, tgSend) {
  const doc = msg.document;
  if (!doc) return false;
  const name = String(doc.file_name || "").toLowerCase();
  if (!name.endsWith(".json")) return false;

  await tgSend(adminChat, "⏳ JSON o'qilmoqda...");
  const file = await ctx.tg.getFile(doc.file_id);
  const buf = await ctx.tg.downloadFile(file.file_path);
  let parsed;
  try {
    parsed = JSON.parse(buf.toString("utf8"));
  } catch {
    return tgSend(adminChat, "⚠️ JSON o'qib bo'lmadi.", {
      reply_markup: adminReportsReplyKeyboard(),
    });
  }

  let imported = null;
  if (parsed?.source === "face-id-terminal" || parsed?.days?.[0]?.staff_key) {
    imported = importDeviceBackfillJson(parsed, employees);
  } else {
    imported = backfillFromTelegramExport(parsed, employees);
  }
  const restored = restoreAllAttendanceData(employees);
  return tgSend(adminChat, formatRestoreReport(restored, imported), {
    reply_markup: adminReportsReplyKeyboard(),
  });
}
...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    if (msg.forward_date || msg.forward_origin) {
      return tryImportForwardedKeldiKetdi(msg, employees, adminChat, tgSend);
    }
    if (msg.document) {
      return handleTelegramExportUpload(msg, ctx, employees, adminChat, tgSend);
    }
    if (!text) return;
    ...
    if (text === "🔄 Ma'lumot tiklash") {
      const restored = restoreAllAttendanceData(employees);
      return tgSend(
        adminChat,
        formatRestoreReport(restored) +
          "\n\n<b>Eski kunlar:</b> guruhdagi KETDI xabarlarini shu botga <b>forward</b> qiling (telefondan ham bo'ladi).",
        { reply_markup: adminReportsReplyKeyboard() }
      );
    }

    if (text === "⚠️ Kelmadi") {
      return tgSend(adminChat, "⚠️ <b>Kelmadi</b>\nKelmagan hodimni tanlang:", {
        reply_markup: absenceTestPickKeyboard(employees),
      });
    }

    if (text === "📝 Kun kiritish") {
      return startFixDayPick(ctx, uid, adminChat, tgSend, employees);
    }
    ...
    if (pending?.mode === "tabel_from_month") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      const ym = pending.monthMap?.[text];
      if (!ym) return;
      const pick = buildDayPickKeyboard(ym);
      ctx.pending.set(uid, { mode: "tabel_from_day", fromYm: ym, dayMap: pick.dayMap });
      return tgSend(
        chatId,
        `📋 <b>TABEL</b>\n\n📅 Boshlanish oyi: <b>${monthPickLabel(ym)}</b>\n\n2️⃣ <b>Boshlanish kunini</b> tanlang:`,
        { reply_markup: pickReplyMarkup(pick) }
      );
    }

    if (pending?.mode === "tabel_from_day") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      if (text === TABEL_BACK) return startTabelPick(ctx, uid, chatId, tgSend);
      const fromDay = pending.dayMap?.[text];
      if (!fromDay) return;
      const pick = buildMonthPickKeyboard({ withBack: true });
      ctx.pending.set(uid, {
        mode: "tabel_to_month",
        fromYm: pending.fromYm,
        fromDay,
        monthMap: pick.monthMap,
      });
      return tgSend(
        chatId,
        `📋 <b>TABEL</b>\n\n▶️ Boshlanish: <b>${fromDay}</b> ${monthPickLabel(pending.fromYm)}\n\n3️⃣ <b>Tugash oyni</b> tanlang:`,
        { reply_markup: pickReplyMarkup(pick) }
      );
    }

    if (pending?.mode === "tabel_to_month") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      if (text === TABEL_BACK) {
        const pick = buildDayPickKeyboard(pending.fromYm);
        ctx.pending.set(uid, { mode: "tabel_from_day", fromYm: pending.fromYm, dayMap: pick.dayMap });
        return tgSend(
          chatId,
          `📋 <b>TABEL</b>\n\n📅 Boshlanish oyi: <b>${monthPickLabel(pending.fromYm)}</b>\n\n2️⃣ <b>Boshlanish kunini</b> tanlang:`,
          { reply_markup: pickReplyMarkup(pick) }
        );
      }
      const toYm = pending.monthMap?.[text];
      if (!toYm) return;
      const pick = buildDayPickKeyboard(toYm);
      ctx.pending.set(uid, {
        mode: "tabel_to_day",
        fromYm: pending.fromYm,
        fromDay: pending.fromDay,
        toYm,
        dayMap: pick.dayMap,
      });
      return tgSend(
        chatId,
        `📋 <b>TABEL</b>\n\n▶️ Boshlanish: <b>${pending.fromDay}</b> ${monthPickLabel(pending.fromYm)}\n` +
          `📅 Tugash oyi: <b>${monthPickLabel(toYm)}</b>\n\n4️⃣ <b>Tugash kunini</b> tanlang:`,
        { reply_markup: pickReplyMarkup(pick) }
      );
    }

    if (pending?.mode === "tabel_to_day") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      if (text === TABEL_BACK) {
        const pick = buildMonthPickKeyboard({ withBack: true });
        ctx.pending.set(uid, {
      ...
      if (!toDay) return;
      const fromKey = dayKeyFromParts(pending.fromYm, pending.fromDay);
      const toKey = dayKeyFromParts(pending.toYm, toDay);
      if (fromKey > toKey) {
        return tgSend(
          chatId,
          `⚠️ Tugash sanasi boshlanishdan oldin.\n` +
            `Boshlanish: ${pending.fromDay} ${monthPickLabel(pending.fromYm)}\n` +
            `Tugash: ${toDay} ${monthPickLabel(pending.toYm)}`
        );
      }
    ...
    if (pending?.mode === "fixday_staff") {
      if (text === "📊 Hisobotlar") return showReportsMenu(ctx, uid, adminChat, tgSend);
      const staffKey = staffKeyByReplyLabel(text, employees);
      if (staffKey) return startFixDayMonth(ctx, uid, adminChat, tgSend, employees, staffKey);
      return;
    }

    if (pending?.mode === "fixday_month") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      if (text === TABEL_BACK) return startFixDayPick(ctx, uid, adminChat, tgSend, employees);
      const ym = pending.monthMap?.[text];
      if (!ym) return;
      const pick = buildDayPickKeyboard(ym);
      ctx.pending.set(uid, {
    ...
    if (pending?.mode === "fixday_day") {
      if (text === "❌ Bekor") return showReportsMenu(ctx, uid, adminChat, tgSend);
      if (text === TABEL_BACK) {
        return startFixDayMonth(ctx, uid, adminChat, tgSend, employees, pending.staffKey);
      }
      const dayNum = pending.dayMap?.[text];
      if (!dayNum) return;
      const dk = dayKeyFromParts(pending.ym, dayNum);
      const staffKey = pending.staffKey;
      const name = staffReplyLabel(employees.staff?.[staffKey]);
      const plan = startFixDayTimeState(staffKey, dk);
      if (plan.complete) {
        return tgSend(chatId, buildFixDayCompleteMessage(name, dk, plan.snap), {
          reply_markup: adminReportsReplyKeyboard(),
        });
      }
      ctx.pending.set(uid, {
      ...
      return tgSend(chatId, buildFixDayTimePrompt(name, dk, plan), {
        reply_markup: adminReportsReplyKeyboard(),
      });
    }

    if (pending?.mode === "fixday_time") {
      const hhmm = normHHMM(text);
      if (!hhmm) {
        const hint = pending.ask === "ketdi" ? "21:00" : "09:15";
        return tgSend(chatId, `⚠️ Format: <code>${hint}</code> (HH:MM)`);
      }
      const staffKey = pending.staffKey;
      const dk = pending.dayKey;
      const staffName = staffReplyLabel(employees.staff?.[staffKey]);

      if (pending.ask === "keldi") {
        pending.keldi = hhmm;
        if (pending.ketdi) {
          const r = saveManualDayKeldiKetdi(staffKey, staffName, dk, pending.keldi, pending.ketdi, employees);
          ctx.pending?.delete(uid);
          if (!r.ok) {
            return tgSend(chatId, `⚠️ ${r.error}`, { reply_markup: adminReportsReplyKeyboard() });
          }
          return tgSend(chatId, formatManualDaySavedMessage(r, staffName, dk), {
            reply_markup: adminReportsReplyKeyboard(),
          });
        }
        ctx.pending.set(uid, { ...pending, ask: "ketdi" });
        return tgSend(
          chatId,
          buildFixDayTimePrompt(staffName, dk, { ...pending, keldi: hhmm, ask: "ketdi" })
        );
      }
    ...
    const staffKey = staffKeyByReplyLabel(text, employees);
    if (staffKey && pending?.mode !== "fixday_staff") {
...
export function adminWelcome() {
  return (
    ...
    "⚠️ Kelmadi — kelmagan hodimga xabar yuborish (Входной / Sababsiz)\n" +
