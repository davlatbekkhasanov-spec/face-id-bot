...
async function handleTelegramExportUpload(msg, ctx, employees, adminChat, tgSend) {
  ...
  await tgSend(adminChat, "⏳ Telegram export o'qilmoqda...");
  ...
  const imported = backfillFromTelegramExport(parsed, employees);
    ...
    if (text === "🔄 Ma'lumot tiklash") {
      ...
      return tgSend(
        ...
        formatRestoreReport(restored) +
          "\n\n<i>Eski kunlar uchun: Telegram Desktop → guruh → Export → result.json ni shu yerga yuboring.</i>",
        { reply_markup: adminReportsReplyKeyboard() }
      );
    }
