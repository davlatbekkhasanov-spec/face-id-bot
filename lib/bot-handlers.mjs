...
import { buildTimesheetReports } from "./timesheet-report.mjs";
import {
  buildDaysListPhoto,
  buildDaysListText,
...
export async function handleMessage(msg, ctx) {
  ...
  if (isAdmin(uid, adminIds)) {
    ...
    if (REPORT_BUTTONS[text]) {
      ...
      if (kind === "timesheet") {
        try {
          const report = await buildTimesheetReports(employees);
          if (!report.images.length) {
            return tgSend(adminChat, report.emptyMessage || "Ma'lumot yo'q.", {
              reply_markup: adminReportsReplyKeyboard(),
            });
          }
          await tgSend(adminChat, report.caption, { reply_markup: adminReportsReplyKeyboard() });
          for (const img of report.images) {
            await ctx.tgSendPhoto(adminChat, img.png, img.caption);
          }
          return;
        } catch (e) {
