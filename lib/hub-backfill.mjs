...
import { isGhostNightScan } from "./shifts.mjs";
...
function dayAlreadyHasRealTimes(staffKey, dayKey, staff) {
  const left = getDb()
    .prepare(
      `SELECT first_in_ms, last_out_ms FROM attendance_log
       WHERE staff_key=? AND day_key=? AND kind='left' AND first_in_ms IS NOT NULL
  ...
  if (
    left?.first_in_ms &&
    left?.last_out_ms &&
    !isGhostNightScan(left.first_in_ms, staff, staffKey)
  ) {
    return true;
  }
  const ds = getDb()
    .prepare(`SELECT first_in_ms, last_out_ms FROM daily_stats WHERE staff_key=? AND day_key=?`)
    .get(staffKey, dayKey);
  if (
    ds?.first_in_ms &&
    ds?.last_out_ms &&
    !isGhostNightScan(ds.first_in_ms, staff, staffKey)
  ) {
    return true;
  }
  return false;
...
function importHubDay(employees, staffKey, staff, staffName, dayKey, keldi, ketdi, workedMsHint) {
  if (dayAlreadyHasRealTimes(staffKey, dayKey, staff)) return false;
