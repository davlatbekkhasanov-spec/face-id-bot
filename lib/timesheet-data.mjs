...
function enrichDayTimes(day, staffEntry, employees) {
  ...
  if (day.firstIn && day.lastOut && !day.workedMs) {
    day.workedMs = Math.max(0, day.lastOut - day.firstIn);
  }
  return day;
}

function fillDayShiftMetrics(day, staffKey, employees) {
  if (!day.firstIn || !hasShiftTracking(employees.staff?.[staffKey])) return day;
  ...
  if (!day.lateMs) day.lateMs = m.lateMs;
  if (!day.debtMs) day.debtMs = m.debtMs;
  if (!day.overtimeMs) day.overtimeMs = m.overtimeMs;
  if (m.workedMs > (day.workedMs || 0)) day.workedMs = m.workedMs;
  return day;
}
...
export function buildTimesheetRange(employees, fromKey, toKey) {
  ...
  for (const r of getAttendanceLogSummariesInRange(fromKey, toKey)) {
    ...
    if (!st) continue;
    const day = st.days.get(r.day_key) || emptyDay(r.day_key);
    if (r.first_in_ms && !day.firstIn) day.firstIn = r.first_in_ms;
    if (r.last_out_ms && !day.lastOut) day.lastOut = r.last_out_ms;
