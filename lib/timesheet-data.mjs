...
function enrichDayTimes(day, staffEntry, employees) {
  ...
  if (day.firstIn && day.lastOut) {
    if (!day.workedMs) day.workedMs = Math.max(0, day.lastOut - day.firstIn);
    return day;
  }
...
function fillDayShiftMetrics(day, staffKey, employees) {
  ...
  if (!day.firstIn && !day.workedMs) return day;
  ...
  if (day.firstIn && (day.lastOut || m.workedMs > 0)) {
    day.lateMs = m.lateMs;
    day.debtMs = m.debtMs;
    day.overtimeMs = m.overtimeMs;
    day.workedMs = m.workedMs;
    if (m.lastOutMs && !day.lastOut) day.lastOut = m.lastOutMs;
  }
  return day;
}
