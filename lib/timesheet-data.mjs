...
import { hasShiftTracking, shiftHoursFor, isGhostNightScan } from "./shifts.mjs";
import {
  dayKey,
  dayKeysInRange,
  dayNumberFromKey,
...
/** Keldi + ketdi ikkalasi bor — tabel va jami hisob uchun */
export function dayIsComplete(day) {
  return Boolean(day?.hasRealIn && day?.hasRealOut && day?.firstIn && day?.lastOut);
}

function mergeTodayFromState(staffKey, dk, day, staff) {
  const st = getStaffState(staffKey);
  if (!st || st.day_key !== dk) return day;
  const dayInIsGhost =
    day.firstIn && staff && isGhostNightScan(day.firstIn, staff, staffKey);
  if (st.first_in_ms) {
    const stateOk = !staff || !isGhostNightScan(st.first_in_ms, staff, staffKey);
    if (stateOk && (!day.firstIn || dayInIsGhost)) {
...
function stripGhostIn(day, staffKey, staff) {
  if (day.firstIn && staff && isGhostNightScan(day.firstIn, staff, staffKey)) {
    day.firstIn = null;
    day.hasRealIn = false;
  }
  return day;
...
export function buildTimesheetRange(employees, fromKey, toKey) {
  ...
  for (const r of rows) {
    ...
    stripGhostIn(day, r.staff_key, st.staff);
    st.days.set(r.day_key, day);
  }

  for (const r of getAttendanceLogSummariesInRange(fromKey, toKey, employees)) {
    const st = byStaff.get(r.staff_key);
    if (!st) continue;
    const day = st.days.get(r.day_key) || emptyDay(r.day_key);
    if (r.first_in_ms) {
  ...
  for (const st of byStaff.values()) {
    ...
    for (const dk of dayKeys) {
      ...
      if (dk === today) day = mergeTodayFromState(st.staffKey, dk, day, st.staff);
      ...
      if (!dayIsComplete(day)) {
        ...
        day.debtMs = 0;
        day.overtimeMs = 0;
        if (!day.hasRealOut) day.workedMs = 0;
      }
      /** Minus = 12 soat − ishlangan (kechikish alohida emas — ish vaqtida aks etadi) */
      const minusMs = day.debtMs || 0;
      cumLate += minusMs;
...
export function staffDetailRows(staffEntry) {
  ...
  for (const day of staffEntry.days.values()) {
    if (!dayIsComplete(day) && !day.hasRealIn && !day.hasRealOut) continue;
    rows.push({
      dayNum: day.dayNum,
      dayKey: day.dayKey,
