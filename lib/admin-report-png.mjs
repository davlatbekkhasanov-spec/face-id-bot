...
function buildSvg(data) {
  ...
  if (data.empty) {
    ...
    body += `<text x="${W / 2}" y="${headerH + kpiH + 62}" text-anchor="middle" fill="${theme.accent}" font-size="28" font-weight="700" font-family="Segoe UI,Arial,sans-serif">${esc(data.emptyMessage)}</text>`;
  } else {
    rows.forEach((row, i) => {
      ...
      body += `<text x="${PAD + 34}" y="${y + 43}" text-anchor="middle" fill="${rankColor}" font-size="20" font-weight="800" font-family="Segoe UI,Arial,sans-serif">${esc(row.rank)}</text>`;
      body += `<text x="${PAD + 72}" y="${y + 32}" fill="#f8fafc" font-size="24" font-weight="700" font-family="Segoe UI,Arial,sans-serif">${esc(truncate(row.name))}</text>`;
      if (row.sub) {
        body += `<text x="${PAD + 72}" y="${y + 56}" fill="#94a3b8" font-size="18" font-family="Segoe UI,Arial,sans-serif">${esc(truncate(row.sub, 34))}</text>`;
      }
      body += `<text x="${W - PAD - 16}" y="${y + 40}" text-anchor="end" fill="${theme.accent}" font-size="24" font-weight="800" font-family="Segoe UI,Arial,sans-serif">${esc(truncate(row.value, 16))}</text>`;
    });
    if (extra > 0) {
      const y = headerH + kpiH + 8 + rows.length * ROW_H;
      body += `<text x="${W / 2}" y="${y + 8}" text-anchor="middle" fill="#64748b" font-size="18" font-family="Segoe UI,Arial,sans-serif">+ yana ${extra} ta</text>`;
    }
  }
  ...
  kpis.forEach((kpi, i) => {
    ...
    kpiSvg += `<text x="${x + 18}" y="${y + 30}" fill="#94a3b8" font-size="16" font-weight="600" font-family="Segoe UI,Arial,sans-serif">${esc(kpi.label)}</text>`;
    kpiSvg += `<text x="${x + 18}" y="${y + 62}" fill="#ffffff" font-size="24" font-weight="800" font-family="Segoe UI,Arial,sans-serif">${esc(truncate(kpi.value, 14))}</text>`;
  });
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="${PAD + (logo ? 104 : 96)}" y="68" fill="#ffffff" font-size="30" font-weight="800" font-family="Segoe UI,Arial,sans-serif">${esc(data.title)}</text>
  <text x="${PAD + (logo ? 104 : 96)}" y="98" fill="#94a3b8" font-size="18" font-family="Segoe UI,Arial,sans-serif">${esc(data.subtitle)} · ${esc(data.dateLabel)} · ${esc(data.timeLabel)}</text>
  <rect x="${W - PAD - 92}" y="48" width="76" height="34" rx="10" fill="${theme.chip}" stroke="${theme.accent}" stroke-opacity="0.35"/>
  <text x="${W - PAD - 54}" y="71" text-anchor="middle" fill="${theme.accent}" font-size="14" font-weight="800" font-family="Segoe UI,Arial,sans-serif">${esc(theme.badge)}</text>
  ${kpiSvg}
  ${body}
  <line x1="${PAD}" y1="${H - footerH}" x2="${W - PAD}" y2="${H - footerH}" stroke="rgba(255,255,255,0.08)" stroke-width="2"/>
  <text x="${PAD}" y="${H - 24}" fill="#64748b" font-size="16" font-family="Segoe UI,Arial,sans-serif">KANSTIK · Face ID hisobot</text>
  <text x="${W - PAD}" y="${H - 24}" text-anchor="end" fill="#64748b" font-size="16" font-family="Segoe UI,Arial,sans-serif">${esc(data.timeLabel)}</text>
</svg>`;
}
