...
function upsertDayFromEvents(staffKey, staffName, dayKey, periodKey, events, employees) {
  ...
  const worked =
    workedMs > 0 ? workedMs : firstIn && lastOut ? Math.max(0, lastOut - firstIn) : 0;
...
export function syncRealTimesFromLogToDailyStats(employees) {
  const rows = getDb()
    .prepare(
      `SELECT staff_key, day_key,
              MIN(CASE WHEN kind IN ('arrived','returned') THEN COALESCE(first_in_ms, event_ms) END) AS first_in,
              MAX(CASE WHEN kind = 'left' THEN COALESCE(last_out_ms, event_ms) END) AS last_out,
              MAX(CASE WHEN kind = 'left' THEN worked_ms ELSE 0 END) AS worked_ms,
  ...
  for (const r of rows) {
    ...
    if (!staff) continue;
    const worked = r.worked_ms > 0 ? r.worked_ms : Math.max(0, r.last_out - r.first_in);
    const metrics = computeDayShiftMetrics(staff, r.staff_key, {
      firstInMs: r.first_in,
      lastOutMs: r.last_out,
      workedMs: worked,
    });
    applyDayShiftMetrics(
      r.staff_key,
      r.staff_name || "",
      r.day_key,
      { ...metrics, firstInMs: r.first_in, lastOutMs: r.last_out },
      employees
    );
    fixed += 1;
  }
