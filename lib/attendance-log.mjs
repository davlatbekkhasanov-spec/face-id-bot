...
import { shiftMsFor, isGhostNightScan } from "./shifts.mjs";
...
/** KETDI first_in_ms ustun; ghost 00:xx emas */
export function resolveDayTimesFromLogEvents(events, staff, staffKey) {
  let firstIn = null;
  let lastOut = null;
  let workedMs = 0;

  for (const ev of events) {
    if (ev.kind !== "left") continue;
    lastOut = ev.last_out_ms || ev.event_ms;
    if (ev.worked_ms > 0) workedMs = ev.worked_ms;
    if (ev.first_in_ms) firstIn = ev.first_in_ms;
  }

  if (!firstIn) {
    for (const ev of events) {
      if (ev.kind !== "arrived" && ev.kind !== "returned") continue;
      const candidate = ev.first_in_ms || ev.event_ms;
      if (!candidate) continue;
      if (staff && isGhostNightScan(candidate, staff, staffKey)) continue;
      if (!firstIn || candidate < firstIn) firstIn = candidate;
    }
  }

  if (firstIn && staff && isGhostNightScan(firstIn, staff, staffKey)) {
    firstIn = null;
    lastOut = null;
    workedMs = 0;
  }

  return { firstIn, lastOut, workedMs };
}

function upsertDayFromEvents(staffKey, staffName, dayKey, periodKey, events, employees) {
  const staff = employees.staff?.[staffKey];
  if (!staff) return false;
  const { firstIn, lastOut, workedMs: logWorked } = resolveDayTimesFromLogEvents(
    events,
    staff,
    staffKey
  );

  if (!firstIn && !lastOut && logWorked <= 0) return false;

  const worked =
    logWorked > 0 ? logWorked : firstIn && lastOut ? Math.max(0, lastOut - firstIn) : 0;
  ...
  if (firstIn && lastOut) {
    ...
    applyDayShiftMetrics(
      ...
      staffName,
      dayKey,
      { ...metrics, firstInMs: firstIn, lastOutMs: lastOut },
      employees
    );
    return true;
  }
  ...
  finalizeDayDebt(staffKey, staffName, dayKey, periodKey, worked, shiftMs, {
    firstInMs: firstIn,
    lastOutMs: lastOut,
  });
...
/** Guruh xabarlaridagi haqiqiy vaqtlarni daily_stats ga (taxmin emas) */
export function syncRealTimesFromLogToDailyStats(employees) {
  const groups = getDb()
    .prepare(
      `SELECT staff_key, day_key, MAX(staff_name) AS staff_name
       FROM attendance_log
       GROUP BY staff_key, day_key`
    )
    .all();
  ...
  let fixed = 0;
  for (const g of groups) {
    const staff = employees.staff?.[g.staff_key];
    if (!staff) continue;
    const events = getDb()
      .prepare(
        `SELECT kind, event_ms, worked_ms, first_in_ms, last_out_ms
    ...
    const worked = logWorked > 0 ? logWorked : Math.max(0, lastOut - firstIn);
    const metrics = computeDayShiftMetrics(staff, g.staff_key, {
      firstInMs: firstIn,
      lastOutMs: lastOut,
      workedMs: worked,
    });
...
export function getAttendanceLogSummariesInRange(fromKey, toKey, employees) {
  const groups = getDb()
    .prepare(
  ...
  for (const g of groups) {
    ...
    const events = getDb()
      ...
      .all(g.staff_key, g.day_key);
    const { firstIn, lastOut, workedMs } = resolveDayTimesFromLogEvents(
      events,
      staff,
      g.staff_key
    );
...
export function restoreAllAttendanceData(employees) {
  ...
  const realTimes = syncRealTimesFromLogToDailyStats(employees);
  const fromState = syncStaffStateToDailyStats(employees);
  return { fromLog, realTimes, fromState };
}
