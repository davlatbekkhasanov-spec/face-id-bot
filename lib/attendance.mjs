...
import { shiftMsFor, shiftStartFor, shiftEndFor, shiftHoursFor, formatShiftLabel, hasShiftTracking, calcLateMs, formatLateLabel, calcEarlyMs, formatEarlyLabel, calcOvertimeMs, formatOvertimeLabel, calcBreakMs, formatBreakLabel } from "./shifts.mjs";
import {
  getDb,
  getStaffState,
  saveStaffState,
...
import { enrichCardWithPoints } from "./points.mjs";
import {
  isFaceEvent,
  staffKey,
  displayName,
...
function attachShiftExtras(card, staff, key, kind, eventMs, rec) {
  ...
  if (kind === "left") {
    card.overtimeMs = calcOvertimeMs(staff, key, eventMs);
...
export function buildAttendanceCard(ev, employees) {
  ...
  return getDb().transaction(() => {
    ...
    if (rec.status !== "in") {
      ...
      if (kind === "arrived") rec.firstInMs = now;
    ...
    attachShiftExtras(card, staff, key, kind, now, rec);
    if (kind === "returned") {
    ...
    enrichCardWithPoints(card, staff);
...
export function buildManualAttendanceCard(staffKey, employees, intent, clockHHMM = null) {
  ...
  return getDb().transaction(() => {
    ...
    if (intent === "in") {
      ...
      kind = trackShift && (rec.hadLeave || rec.workedMs > 0) ? "returned" : "arrived";
      if (kind === "arrived") rec.firstInMs = eventMs;
    } else {
