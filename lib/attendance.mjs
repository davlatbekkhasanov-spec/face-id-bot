...
import { computePeriodBalanceMs } from "./timesheet-balance.mjs";
...
export function buildAttendanceCard(ev, employees) {
  ...
  const card = getDb().transaction(() => {
    ...
    if (rec.status !== "in") {
      ...
      clearAbsenceIfArrived(key, dk, who);
      saveStaffState(recToRow(key, who, rec));
    } else {
  ...
  if (!card) return null;
  applyPointsToCard(card, staff, employees);
  logAttendanceCard(card);
  card._logDone = true;
  if (employees && hasShiftTracking(staff)) {
    card.monthBalanceMs = computePeriodBalanceMs(employees, card.staffKey, card.periodKey);
  }
...
export function buildManualAttendanceCard(staffKey, employees, intent, clockHHMM = null) {
  ...
  const tx = getDb().transaction(() => {
    ...
    if (intent === "in") {
      ...
      if (intent === "in") clearAbsenceIfArrived(staffKey, dk, who);
    } else {
  ...
  applyPointsToCard(result.card, staff, employees);
  logAttendanceCard(result.card);
  result.card._logDone = true;
  if (employees && hasShiftTracking(staff)) {
    result.card.monthBalanceMs = computePeriodBalanceMs(
      employees,
      result.card.staffKey,
      result.card.periodKey
    );
  }
