...
/** Guruh xabari uchun to'liq karta (null = cooldown yoki begona) */
export function buildAttendanceCard(ev, employees) {
  ...
  let kind;
  let dayDebtMs = Math.max(0, SHIFT_MS - rec.workedMs);
  ...
  if (rec.status !== "in") {
    ...
    rec.hadLeave = true;
    kind = "left";
    dayDebtMs = finalizeDayDebt(key, who, dk, pk, rec.workedMs);
