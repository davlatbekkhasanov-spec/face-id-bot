...
import { shiftMsFor, shiftStartFor, shiftEndFor, shiftHoursFor, formatShiftLabel, hasShiftTracking, calcLateMs, formatLateLabel, calcEarlyMs, formatEarlyLabel, calcOvertimeMs, formatOvertimeLabel, calcEarlyLeaveMs, formatEarlyLeaveLabel, calcBreakMs, formatBreakLabel } from "./shifts.mjs";
import {
  getDb,
  getStaffState,
  saveStaffState,
...
export { isFaceEvent, loadEmployees, staffKey, displayName, eventTimeMs, fmtClock, parseClockToday } from "./attendance-core.mjs";
...
export function buildAttendanceCard(ev, employees) {
  ...
  return getDb().transaction(() => {
    ...
    if (rec.status !== "in") {
      ...
      sessionMs = Math.max(0, wall - (rec.sessionStartMs || wall));
    ...
    if (trackShift) {
      ...
      if (kind === "left") {
        card.sessionMs = sessionMs;
        card.overtimeMs = calcOvertimeMs(staff, key, now);
        card.overtimeLabel = formatOvertimeLabel(card.overtimeMs);
        card.earlyLeaveMs = calcEarlyLeaveMs(staff, key, now);
        card.earlyLeaveLabel = formatEarlyLeaveLabel(card.earlyLeaveMs);
      }
    }
...
export function buildManualAttendanceCard(staffKey, employees, intent, clockHHMM = null) {
  ...
  return getDb().transaction(() => {
    ...
    if (trackShift) {
      card.shiftStart = shiftStartFor(staff, staffKey);
      card.shiftEnd = shiftEndFor(staff, staffKey);
      card.shiftHours = shiftHoursFor(staff);
      card.shiftLabel = formatShiftLabel(staff, staffKey);
      if (kind === "arrived") {
        card.lateMs = calcLateMs(staff, staffKey, eventMs);
        card.lateLabel = formatLateLabel(card.lateMs);
        card.earlyMs = calcEarlyMs(staff, staffKey, eventMs);
        card.earlyLabel = formatEarlyLabel(card.earlyMs);
      }
      if (kind === "returned") {
        card.breakMs = breakMs;
      ...
      if (kind === "left") {
        card.sessionMs = sessionMs;
        card.overtimeMs = calcOvertimeMs(staff, staffKey, eventMs);
        card.overtimeLabel = formatOvertimeLabel(card.overtimeMs);
        card.earlyLeaveMs = calcEarlyLeaveMs(staff, staffKey, eventMs);
        card.earlyLeaveLabel = formatEarlyLeaveLabel(card.earlyLeaveMs);
      }
    }
