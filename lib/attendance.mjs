...
function fmtHours(ms) {
  ...
  const whole = Math.floor(totalMin / 60);
  const m = totalMin % 60;
  if (whole === 0) return `${m} daqiqa`;
  if (m === 0) return `${whole} soat`;
  return `${whole} soat ${m} daq`;
}
...
function tgLine(employees, key) {
  const tg = employees.staff?.[key]?.telegramId;
  return tg ? `\n<code>tg:${tg}</code>` : "";
}

/** Bugun: keldi → ketdi → ishga qaytdi → ketdi (vaqt jamlanadi) */
export function buildMessage(ev, state, employees) {
  ...
  if (rec.status !== "in") {
    ...
    let msg = back
      ? `🔄 <b>${who}</b> ishga qaytdi — <b>${clock}</b>`
      : `📥 <b>${who}</b> keldi — <b>${clock}</b>`;
    if (back && rec.workedMs > 0) {
      msg += `\nBugun jami: <b>${fmtHours(rec.workedMs)}</b>`;
    }
    return msg + tgLine(employees, key);
  }

  const sessionMs = now - (rec.sessionStart || now);
  rec.workedMs += sessionMs;
  rec.status = "out";
  rec.sessionStart = null;
  rec.hadLeave = true;

  const debt = Math.max(0, SHIFT_MS - rec.workedMs);
