...
import { checkAbsenceAlerts } from "./absence-alerts.mjs";
import { checkShiftReminders } from "./reminders.mjs";
...
export function startTelegramPoll(ctx) {
  ...
  const pollCtx = {
    ...
    tgEdit: (chatId, msgId, text, extra) => tg.edit(chatId, msgId, text, extra).catch(() => {}),
  };

  let lastAbsenceCheck = 0;
  ...
  (async function loop() {
    for (;;) {
      try {
        const now = Date.now();
        if (now - lastAbsenceCheck > 25_000) {
          lastAbsenceCheck = now;
          checkAbsenceAlerts(pollCtx).catch((e) => console.warn("absence:", e.message));
          checkShiftReminders(ctx.employees, (chatId, text) =>
            pollCtx.tgSend(chatId, text).catch(() => {})
          );
        }
