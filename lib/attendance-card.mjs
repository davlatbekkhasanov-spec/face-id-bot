...
function appendLeaveSummary(text, card) {
  ...
  if (card.overtimeMs > 0 && card.overtimeLabel) {
    text += `\n💪 ${b("Smenadan keyin")}: ${b(card.overtimeLabel)}`;
  }
...
/** Guruhga: faqat keldi/ketdi (kunlik natija va ball yo'q) */
export function formatKeldiKetdiCaption(card) {
...
/** Admin lichka: to'liq kunlik natija + ball */
export function formatShortCaption(card) {
  let text = formatKeldiKetdiCaption(card);
...
export function formatGroupCaption(card) {
  return formatKeldiKetdiCaption(card);
}
