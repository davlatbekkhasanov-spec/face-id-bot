...
import { formatPointsSigned } from "./points.mjs";
import { pointsEnabled } from "./points-config.mjs";
import { fmtMoneySom } from "./timesheet-salary.mjs";
...
function appendLeaveSummary(text, card) {
  ...
  const salary = card.monthlySalary || 0;
  const dayDebtMs = card.dayDebtMs || 0;
  const overtimeMs = card.overtimeMs || 0;
  const dayBalMs = overtimeMs - dayDebtMs;

  if (dayDebtMs <= 60_000 && dayBalMs <= 0) {
    text += `\n🟢 ${b("Me'yor bajarildi")}`;
    if (overtimeMs > 0 && card.overtimeLabel) {
  ...
  if (salary) {
    if (dayBalMs < 0) {
      text += `\n💰 ${b("Kunlik qarzingiz")}: ${b(fmtMoneySom(salary, dayBalMs))}`;
    }
    if (dayBalMs > 0) {
      text += `\n➕ ${b("Balans +")}: ${b(fmtMoneySom(salary, dayBalMs))}`;
    }
    const monthBalMs = card.monthBalanceMs ?? dayBalMs;
    const monthMoney = fmtMoneySom(salary, monthBalMs);
    if (monthMoney) {
      const monthLabel = monthBalMs < 0 ? "Oylik qarzingiz" : "Oylik balans";
      text += `\n📊 ${b(monthLabel)}: ${b(monthMoney)}`;
    }
  } else if (dayDebtMs > 60_000) {
    text += `\n💰 ${b("Kunlik qarzingiz")}: ${b(fmtDurationNorm(dayDebtMs))}`;
  }

  if (pointsEnabled() && card.dayPoints != null) {
    text += `\n📊 ${b("Ball")}: ${b(formatPointsSigned(card.dayPoints))}`;
  }
...
export function formatPingBreakdown(card) {
  ...
  if (dayLeft > 60_000) {
    const salary = card.monthlySalary || 0;
    const dayBalMs = (card.overtimeMs || 0) - (card.dayDebtMs || 0);
    const dayMoney = fmtMoneySom(salary, dayBalMs);
    const monthMoney = fmtMoneySom(salary, card.monthBalanceMs ?? 0);
    if (dayMoney) {
      rows.push(`Kunlik qarz           | ${dayMoney.replace(/ so'm$/, "")}`);
      if (monthMoney) rows.push(`Oylik qarz            | ${monthMoney.replace(/ so'm$/, "")}`);
    } else {
      rows.push(
  ...
  } else {    rows.push("Natija                 | Me'yor bajarildi");
  }
