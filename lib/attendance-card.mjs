...
function appendLeaveSummary(text, card) {
  ...
  if (card.dayDebtMs > 60_000) {
    ...
    if (dayMoney) {
      text += `\n💰 ${b("Kunlik qarzingiz")}: ${b(dayMoney)}`;
      if (monthMoney) {
        text += `\n📊 ${b("Oy davomidagi qarzingiz")}: ${b(monthMoney)}`;
      }
    } else {
      text += `\n💰 ${b("Kunlik qarzingiz")}: ${b(fmtDurationNorm(card.dayDebtMs))}`;
    }
  } else {    text += `\n🟢 ${b("Me'yor bajarildi")}`;
    if (card.overtimeMs > 0 && card.overtimeLabel) {
...
export function formatPingBreakdown(card) {
  ...
  if (dayLeft > 60_000) {
    ...
    if (dayMoney) {
      ...
      if (monthMoney) rows.push(`Oy davomidagi qarz    | ${monthMoney.replace(/ so'm$/, "")}`);
    } else {
      rows.push(
