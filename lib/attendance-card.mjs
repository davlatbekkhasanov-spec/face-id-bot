...
export function formatShortCaption(card) {
  ...
  if (card.earlyLeaveMs > 0 && card.earlyLeaveLabel) {
    text += `\n🚪 Erta ketdi: <b>${card.earlyLeaveLabel}</b>`;
  }
  ...
  if (card.kind === "left" && !card.noShift) {
    if (card.sessionMs > 0) {
      text += `\n⏱ Bu safar: <b>${fmtDuration(card.sessionMs)}</b>`;
    }
    text += `\n📊 Bugun ishlagan: <b>${fmtDuration(card.dayWorkedMs)}</b>`;
    if (card.dayDebtMs > 60_000) {
      text += `\n📉 Qolgan qarz: <b>${fmtDuration(card.dayDebtMs)}</b>`;
    } else if (card.shiftMs > 0) {
