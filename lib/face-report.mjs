...
import { normalizePortrait, portraitB64FromBuffer } from "./portrait.mjs";
...
function cardShiftMs(card) {
  return card.shiftMs || SHIFT_MS;
}

function workPercent(card) {
  const sm = cardShiftMs(card);
  return Math.min(100, Math.round((card.dayWorkedMs / sm) * 100));
}

async function portraitB64(photoPath) {
  if (!photoPath || !fs.existsSync(photoPath)) return "";
  try {
    return portraitB64FromBuffer(await normalizePortrait(photoPath, null));
  } catch (e) {
...
function breakdownRows(card) {
  ...
  const normLabel = fmtDuration(sm);
  const rows = [
    ["Кунлик меъёр", normLabel, "—"],
    ["Бугун ишланган", "Фейс ИД", fmtDuration(card.dayWorkedMs)],
  ];
  if (dayLeft > 60_000) {
    rows.push(["Кунлик қарз", `${normLabel} − ${fmtDuration(card.dayWorkedMs)}`, fmtDuration(dayLeft)]);
  } else {
...
function summaryText(card) {
  ...
  if (card.kind === "left") {
    if (dayLeft > 60_000) {
      return `${n} бугунги сменасини якунлади. Ишланган: ${w}. Қарз: ${fmtDuration(dayLeft)}.`;
    }
    return `${n} бугунги сменасини якунлади. Кунлик меъёр бажарилди — ${w}.`;
  }
  if (card.kind === "returned") {
    return `${n} яна ишга қайтди. Бугун ${w}, қолган ${fmtDuration(dayLeft)}.`;
  }
  return `${n} ишга келди. Кунлик меъёр: ${fmtDuration(sm)}.`;
}
...
export async function buildFaceReportHtml(card, photoPath) {
  ...
  const sm = cardShiftMs(card);
  const dayLeft = Math.max(0, sm - card.dayWorkedMs);
  const pct = workPercent(card);
  const month = periodLabel(card.periodKey);
  const shiftStart = card.shiftStart || "—";
  const normLabel = fmtDuration(sm);
  ...
  const unitId = card.staffKey ? `ИД · ${esc(card.staffKey)}` : "СКЛАД-3";
...
<div class="page">
  ...
  <header class="hdr">
    ...
    <div class="hdr-mid">
      ...
      <p>Фейс ИД · KANSTIK SAMARQAND</p>
    </div>
    <div class="hdr-badge">СКЛАД-3</div>
  </header>
  ...
  <div class="chips">
    ...
    <div class="chip">Иш кунлари<b>${esc(String(card.workDays))}</b></div>
      <div class="chip">Бошланиш<b>${esc(shiftStart)}</b></div>
      <div class="chip">Меъёр<b>${esc(normLabel)}</b></div>
  </div>
  ...
  <footer class="ftr">
    <span class="ftr-tag">KANSTIK · Фейс ИД</span>
