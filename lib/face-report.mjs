...
/** Toza, keskin portret — forma overlay YO'Q */
  ...
  try {
    ...
    return (
      await sharp(photoPath)
        .rotate()
        .resize(680, 800, { fit: "cover", position: "top" })
...
function progressRing(pct) {
  ...
  return `<svg viewBox="0 0 260 260">
    <circle cx="130" cy="130" r="${r}" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="14"/>
    <circle cx="130" cy="130" r="${r}" fill="none" stroke="url(#rg)" stroke-width="14"
...
<div class="page">
  ...
  <section class="hero">
    <div class="portrait-wrap">
      ...
      <div class="portrait-box">${photoHtml}</div>
      <div class="unit-badge">${unitId}</div>
    </div>

    <div class="emp">${esc(card.staffName)}</div>
    <div class="status ${statusClass}">${icon} ${esc(kind)}</div>
  ...
  try {
    ...
    try {
      const page = await browser.newPage({
        viewport: { width: 1080, height: 1320 },
        deviceScaleFactor: 2,
      });
      await page.setContent(html, { waitUntil: "networkidle" });
      await page.evaluate(() => document.fonts.ready);
      const height = await page.evaluate(() =>
        Math.max(1320, Math.ceil(document.querySelector(".page").getBoundingClientRect().height) + 8)
      );
