...
export function adminReportsReplyKeyboard() {
  return {
    keyboard: [
      ...
      [{ text: "🏆 Oy reytingi" }, { text: "📋 Tabel" }],
      [{ text: "📆 Ma'lumot kunlari" }, { text: "🗑 Ma'lumot o'chirish" }],
      [{ text: "🔄 Ma'lumot tiklash" }, { text: "⚠️ Kelmadi test" }],
...
export function adminMainKeyboard() {
  return {
    inline_keyboard: [
      [{ text: "✍️ Qo'lda keldi/ketdi", callback_data: "adm:pick" }],
      [{ text: "⚠️ Kelmadi test", callback_data: "adm:abspick" }],
    ],
  };
}
...
/** Kelmadi xabarini qo'lda test — hodim tanlash */
export function absenceTestPickKeyboard(employees) {
  const rows = [];
  const staff = Object.entries(employees.staff || {}).filter(([, s]) => !s?.noShift);
  for (let i = 0; i < staff.length; i += 2) {
    const row = [];
    for (let j = i; j < Math.min(i + 2, staff.length); j++) {
      const [key, s] = staff[j];
      row.push({ text: staffReplyLabel(s), callback_data: `adm:abssend:${key}` });
    }
    rows.push(row);
  }
  rows.push([{ text: "◀️ Orqaga", callback_data: "adm:menu" }]);
