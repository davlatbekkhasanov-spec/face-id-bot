...
/** Hodimga maxsus guruh (employees.json: attendanceGroupId) */
export function staffAttendanceGroupIds(staffKey, employees) {
  const staff = employees?.staff?.[staffKey];
  if (!staff) return null;
  const ids = staff.attendanceGroupIds;
  if (Array.isArray(ids) && ids.length) {
    return ids.map((x) => String(x).trim()).filter(Boolean);
  }
  const one = staff.attendanceGroupId;
  if (one != null && String(one).trim()) return [String(one).trim()];
  return null;
}

/** Keldi/ketdi qayerga — hodim override yoki umumiy guruhlar */
export function resolveAttendanceChatIdsForStaff(ctx, staffKey, employees) {
  const override = staffAttendanceGroupIds(staffKey, employees);
  if (override?.length) return override;
  return resolveAttendanceChatIds(ctx);
}
