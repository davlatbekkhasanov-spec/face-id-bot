...
export function applyPointsToCard(card, staff) {
  ...
  try {
    ...
    if (tgId) {
      pushFaceIdToHub({ telegramId: tgId, dayKey: dk, breakdown: computed }).then((r) => {
...
export function formatPointsBlock(caption, card) {
  ...
  if (card.pointsDelta && card.pointsDelta !== 0) {
    line += ` · ${fmtBold(`bu skan ${formatPointsSigned(card.pointsDelta)}`)}`;
  }
