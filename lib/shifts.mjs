...
/** Ketish: tungi yolg'on kelishni haqiqiy sessiya boshiga almashtirish */
export function repairGhostSessionBeforeLeave(rec, staff, staffKey, now) {
  if (rec.status !== "in") return;
  if (!isGhostNightScan(rec.firstInMs, staff, staffKey)) {
    if (!isGhostNightScan(rec.sessionStartMs, staff, staffKey)) return;
  }

  const shiftStart = shiftStartMsToday(staff, staffKey, now);
  const sessionOk =
    rec.sessionStartMs && !isGhostNightScan(rec.sessionStartMs, staff, staffKey);
  const realStart = sessionOk ? rec.sessionStartMs : shiftStart;

  rec.firstInMs = realStart;
  rec.sessionStartMs = realStart;
  rec.workedMs = 0;
}
