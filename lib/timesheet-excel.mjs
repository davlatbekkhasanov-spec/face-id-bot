...
/** Smenasiz xodim — bo'sh; smenali + Face ID yo'q — В */
export function dayCode(day, dk, staff) {
  if (!hasAttendance(day)) {
    if (dk >= dayKey()) return "";
    if (staff?.noShift) return "";
    return "В";
  }
  return "Я";
}
...
function dayCellContent(day, dk, staff) {
  const code = dayCode(day, dk, staff);
  if (!code) return "";
  if (code === "В") return "В";
  const net = hoursNum(day?.netMs ?? day?.workedMs);
  if (day?.firstIn && day?.lastOut) {
    return `${code}\n${fmtClockMs(day.firstIn)}-${fmtClockMs(day.lastOut)}\n${net ?? ""}`;
  }
  if (net != null) return `${code}\n${net}`;
  return code;
}
...
function buildMainSheet(wb, data) {
  ...
  ws.getCell(3, 1).value =
    "Я — ish · В — Face ID yo'q (smena) · 12s norma · <12s qizil · >12.5s yashil · Soat = ish − kech + ortiqcha";
  ...
  h1.getCell(sumStart + 3).value = "Ortiqcha";
  styleHeaderRow(h1);
  ...
  data.staff.forEach((st, idx) => {
    ...
    days.forEach((dk, i) => {
      ...
      const code = dayCode(day, dk, st);
      const cell = row.getCell(3 + i);
      cell.value = dayCellContent(day, dk, st);
      if (code === "Я") workDays += 1;
      const style = code === "В" ? { off: true } : dayCellStyle(day);
      styleDataCell(cell, style);
    });
    row.getCell(sumStart).value = workDays;
    row.getCell(sumStart + 1).value = hoursNum(st.totalNetMs);
    row.getCell(sumStart + 2).value = minsNum(st.totalLateMs);
    row.getCell(sumStart + 3).value = hoursNum(st.totalOvertimeMs);
...
function buildDetailSheet(wb, data) {
  ...
  for (const st of data.staff) {
    for (const dk of data.days) {
      ...
      line.getCell(3).value = code;
      line.getCell(4).value = day?.firstIn ? fmtClockMs(day.firstIn) : "";
      line.getCell(5).value = day?.lastOut ? fmtClockMs(day.lastOut) : "";
      line.getCell(6).value = hoursNum(day?.workedMs);
      line.getCell(7).value = minsNum(day?.lateMs);
      line.getCell(8).value = hoursNum(day?.overtimeMs);
      line.getCell(9).value = hoursNum(day?.netMs);
      line.getCell(10).value = minsNum(day?.cumLateMs);
      line.getCell(11).value = hoursNum(day?.cumOvertimeMs);
      const style = code === "В" ? { off: true } : dayCellStyle(day);
      for (let c = 1; c <= 11; c++) {
        styleDataCell(line.getCell(c), { center: c > 1, ...style });
      }
      r += 1;
    }
  }
...
export function excelCaption(data) {
  return (
    `📋 <b>TABEL (Excel)</b>\n` +
    `📅 ${data.rangeLabel}\n` +
    `👥 ${data.staff.length} xodim · Soat = ish − kech + ortiqcha`
  );
}
