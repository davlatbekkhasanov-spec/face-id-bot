...
import { calcBalansSum, fmtBalansSum, staffMonthlySalary } from "./timesheet-salary.mjs";
...
/** Bundan oldingi kunlar tabelda avtomatik «В»; keyin faqat admin «Входной» */
const NO_AUTO_EXCUSED_FROM = "2026-07-04";

/** Jami vaqt: 60:30 (soat:daqiqa) — keldi/ketdi daqiqasi bilan mos */
...
function hasAttendance(day) {
  return dayIsComplete(day);
}

function hasRealKeldiKetdi(day) {
  return Boolean(day?.hasRealIn && day?.hasRealOut && day?.firstIn && day?.lastOut);
}

/** Smenasiz — bo'sh; «В»/«Н» faqat admin belgilagan yoki eski avtomatik qoida */
export function dayAbsentMark(day, dk, staff) {
  if (hasAttendance(day)) return null;
  if (day?.absenceMark === "unexcused") return "Н";
  if (day?.absenceMark === "excused") return "В";
  if (staff?.noShift) return null;
  if (dk >= dayKey()) return null;
  if (dk < NO_AUTO_EXCUSED_FROM) return "В";
  return null;
}
...
const OT_MARGIN_H = 0.5;

function dayCellStyle(day, staff) {
  const normH = staff?.shiftHours || NORM_H;
  const h = dayWorkedH(day);
  if (h <= 0) return {};
  if (h >= normH + OT_MARGIN_H) return { ot: true };
  if (h < normH) return { short: true };
  return {};
}
...
/** Yuqori: keldi · Past: ketdi — FAQAT haqiqiy vaqt; taxmin yo'q */
function dayCellContent(day, dk, staff) {
  const absent = dayAbsentMark(day, dk, staff);
  if (absent) return absent;
  if (!hasAttendance(day)) return "";

  if (hasRealKeldiKetdi(day)) {
    return `${fmtClockMs(day.firstIn)}\n${fmtClockMs(day.lastOut)}`;
  }
  if (day.hasRealIn && day.firstIn) {
    return fmtClockMs(day.firstIn);
  }
  if (day.workedMs > 0) {
    return `⏱ ${fmtHoursClock(day.workedMs)}`;
  }
  return "";
}
...
function buildMainSheet(wb, data) {
  ...
  ws.getCell(3, 1).value =
    "Kunlar: keldi/ketdi (haqiqiy) · Minus=12soat−ish · Plus=ortiqcha · Balans=Plus−Minus · Balans SUM=soat×oylik/264soat";
  ...
  h1.getCell(sumStart + 5).value = "Balans SUM";
  styleHeaderRow(h1);
  ...
  h2.getCell(sumStart + 5).value = "so'm";
  [sumStart + 1, sumStart + 2, sumStart + 3, sumStart + 4, sumStart + 5].forEach((c) => {
  ...
  data.staff.forEach((st, idx) => {
    ...
    days.forEach((dk, i) => {
      ...
      const style = absent ? { off: true } : dayCellStyle(day, st);
      styleDataCell(cell, style);
    });
    row.getCell(sumStart).value = Math.max(0, workDays - (st.totalAbsentPenalty || 0));
    row.getCell(sumStart + 1).value = fmtHoursClock(st.totalWorkedMs);
    row.getCell(sumStart + 2).value = fmtHoursClock(st.totalMinusMs);
    row.getCell(sumStart + 3).value = fmtHoursClock(st.totalOvertimeMs);
    const balanceMs =
      st.totalBalanceMs != null
        ? st.totalBalanceMs
        : (Number(st.totalOvertimeMs) || 0) - (Number(st.totalMinusMs) || 0);
    row.getCell(sumStart + 4).value = fmtBalance(balanceMs);
    const salary = staffMonthlySalary(st.staff);
    const balansSum = calcBalansSum(salary, balanceMs);
    row.getCell(sumStart + 5).value = salary ? fmtBalansSum(balansSum) : "—";
    const balCell = row.getCell(sumStart + 4);
    const sumCell = row.getCell(sumStart + 5);
    [1, 2, sumStart, sumStart + 1, sumStart + 2, sumStart + 3, sumStart + 4, sumStart + 5].forEach((c) =>
    ...
    if (balansSum > 0) styleDataCell(sumCell, { ot: true });
    else if (balansSum < 0) styleDataCell(sumCell, { short: true });
  ...
  ws.getColumn(sumStart + 5).width = 14;
...
function buildDetailSheet(wb, data) {
  ...
  for (const st of data.staff) {
    for (const dk of data.days) {
      ...
      let bal = 0;
      if (absent) {
        line.getCell(3).value = absent;
      } else {
        line.getCell(3).value = day?.hasRealIn && day?.firstIn ? fmtClockMs(day.firstIn) : "";
        line.getCell(4).value = day?.hasRealOut && day?.lastOut ? fmtClockMs(day.lastOut) : "";
        line.getCell(5).value = fmtHoursClock(day?.workedMs);
        line.getCell(6).value = fmtHoursClock(day?.minusMs ?? day?.debtMs ?? 0);
        line.getCell(7).value = fmtHoursClock(day?.overtimeMs);
        bal = (day?.overtimeMs || 0) - (day?.minusMs ?? day?.debtMs ?? 0);
