...
function buildMainSheet(wb, data) {
  ...
  ws.getCell(3, 1).value =
    "Har katak: yuqori keldi · past ketdi · В = Face ID yo'q · 12s norma · <12s qizil · >12.5s yashil";
  ...
  h1.getCell(sumStart + 1).value = "Ish (soat)";
  h1.getCell(sumStart + 2).value = "Minus (soat)";
  h1.getCell(sumStart + 3).value = "Plus (soat)";
  styleHeaderRow(h1);
  ...
  data.staff.forEach((st, idx) => {
    ...
    row.getCell(sumStart + 2).value = hoursNum(st.totalLateMs);
...
function buildDetailSheet(wb, data) {
  ...
  for (const st of data.staff) {
    for (const dk of data.days) {
      ...
      if (absent) {
        ...
        line.getCell(6).value = hoursNum(day?.lateMs);
        line.getCell(7).value = hoursNum(day?.overtimeMs);
        line.getCell(8).value = hoursNum(day?.netMs);
      }
      const style = absent ? { off: true } : dayCellStyle(day);
      for (let c = 1; c <= 8; c++) {
  ...
  for (let c = 5; c <= 8; c++) ws.getColumn(c).width = 12;
}
...
export function excelCaption(data) {
  return (
    `📋 <b>TABEL (Excel)</b>\n` +
    `📅 ${data.rangeLabel}\n` +
    `👥 ${data.staff.length} xodim · keldi/ketdi vaqt`
  );
}
