...
function buildMainSheet(wb, data) {
  ...
  ws.getCell(3, 1).value =
    "Kunlar: keldi/ketdi (haqiqiy) · Minus=kech+qarz · Plus=ortiqcha · Balans=Plus−Minus";
  ...
  for (const st of data.staff) {
    for (const dk of data.days) {
      ...
      if (absent) {
        ...
        line.getCell(6).value = fmtHoursClock(day?.minusMs ?? (day?.lateMs || 0) + (day?.debtMs || 0));
        line.getCell(7).value = fmtHoursClock(day?.overtimeMs);
        bal = (day?.overtimeMs || 0) - (day?.minusMs ?? (day?.lateMs || 0) + (day?.debtMs || 0));
