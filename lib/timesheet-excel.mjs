...
function buildMainSheet(wb, data) {
  ...
  ws.getCell(3, 1).value =
    "Kunlar: yuqori keldi · past ketdi · Jami: soat:daqiqa (60:30) · <12s qizil · >12.5s yashil";
  ...
  [sumStart + 1, sumStart + 2, sumStart + 3].forEach((c) => {
  ...
  for (const st of data.staff) {
    for (const dk of data.days) {
      ...
      if (absent) {
        ...
        line.getCell(8).value = fmtHoursClock(day?.netMs);
      }
      const style = absent ? { off: true } : dayCellStyle(day);
      for (let c = 1; c <= 8; c++) {
