...
import { getMonthlyPointsLeaderboard } from "./points-db.mjs";
...
export function collectLeadersReport() {
  const pk = periodKey();
  if (pointsEnabled()) {
    const leaders = getMonthlyPointsLeaderboard(pk, 12);
    const rows = leaders.map((r, i) => ({
      rank: rankLabel(i),
      name: r.staff_name,
      value: r.total_points > 0 ? `+${r.total_points}` : String(r.total_points),
      sub: "Face ID ball · oy yig'indisi",
      tier: i < 3 ? `gold-${i + 1}` : "default",
    }));
    const totalPts = leaders.reduce((sum, r) => sum + (r.total_points || 0), 0);
    return {
      kind: "leaders",
      theme: "violet",
      title: "OY REYTINGI · BALL",
      subtitle: periodLabel(pk),
      dateLabel: fmtDayHuman(dayKey()),
      timeLabel: nowClock(),
      kpis: [
        { label: "Top", value: `${rows.length} xodim` },
        { label: "Jami ball", value: totalPts > 0 ? `+${totalPts}` : String(totalPts) },
        { label: "Lider", value: rows[0]?.name?.split(" ").slice(-1)[0] || "—" },
      ],
      rows,
      empty: rows.length === 0,
      emptyMessage: "Ball ma'lumot yo'q",
    };
  }
