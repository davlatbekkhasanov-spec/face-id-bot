...
export function auditTimesheetData(data) {
  ...
  for (const st of data.staff) {
    ...
    for (const dk of data.days) {
      ...
      const realBoth = day.hasRealIn && day.hasRealOut && day.firstIn && day.lastOut;
      if (realBoth) {
        realTimeDays += 1;
        ...
        partialTimeDays += 1;
        issues.push(`${st.name} ${dk}: ketdi yoki keldi vaqti bazada yo'q`);
      ...
      const expectedBal = (day.overtimeMs || 0) - minus;
      const bal = (day.overtimeMs || 0) - (day.lateMs || 0) - (day.debtMs || 0);
      if (expectedBal !== bal) {
