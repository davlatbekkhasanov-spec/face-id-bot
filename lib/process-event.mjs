...
import { resolveAttendanceChatIdsForStaff } from "./chats.mjs";
...
export async function handleFaceEvent(ev, employees, ctx) {
  ...
  const chatIds = resolveAttendanceChatIdsForStaff(ctx, card.staffKey, employees);
...
export async function sendManualAndStore(dataDir, botToken, ctx, card, employees) {
  ...
  const chatIds = resolveAttendanceChatIdsForStaff({ ...ctx, botToken }, card.staffKey, employees);
